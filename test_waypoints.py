...
class World(object):
    ...
    def restart(self, args):
        ...
        while self.player is None:
            ...
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
            self.player.set_simulate_physics(False) # not simulating physics
...
def game_loop(args):
    ...
    try:
        ...
        wp = map.get_waypoint(world.player.get_location()) # get the waypoint of the player
        ...
        while True:
            ...
            # exploer waypoints
            if count % 10 == 0:
                next_wps = list(wp.next(1.0)) # a list of waypoints that is 1.0m away from the current waypoint
                print('Next(1.0) -->  %d waypoints' % len(next_wps))
                if not next_wps:
                    raise RuntimeError("No next waypoints")

                wp = random.choice(next_wps)
                text = "road id = %d, lane id = %d, transform = %s"
                print(text % (wp.road_id, wp.lane_id, wp.transform))

                # draw the waypointS
                if count % 40 == 0:
                    draw_waypoints(world.world, wp)
                    count = 0
                
                # move car to the next waypoint
                world.player.set_transform(wp.transform)
